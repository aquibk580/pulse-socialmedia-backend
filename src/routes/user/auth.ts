...
type VerifyCallback = (
  ...
  refreshToken: string,
  profile: any,
  done: (error: any, user?: any, info?: { isNew: boolean }) => void
) => void;

const googleStrategyOptions: GoogleStrategyOptions = {
  clientID: process.env.GOOGLE_CLIENT_ID as string,
  clientSecret: process.env.GOOGLE_CLIENT_SECRET as string,
  callbackURL: `${process.env.FRONTEND_URL}/api/auth/google/callback` as string,
  scope: ["profile", "email"],
...
router.get(
  ...
  (req: Request, res: Response, next: NextFunction) => {
    passport.authenticate("google", { session: false }, (err, user, info) => {
      ...
      const { redirectUrl } = JSON.parse(decodeURIComponent(state as string));
